...
def setup_korean_font():
    ...
    # 이렇게 하면 Matplotlib이 폰트 목록을 다시 스캔하게 됩니다.
    cache_dir = fm.get_cachedir()
    
    # cache_dir이 존재하고 디렉토리인지 확인
    if os.path.exists(cache_dir) and os.path.isdir(cache_dir):
        # 'fontlist-'로 시작하고 '.json'으로 끝나는 폰트 캐시 파일들을 찾아 삭제
        # 모든 폰트 캐시 파일을 삭제하여 확실히 재빌드 되도록 합니다.
        for fname in os.listdir(cache_dir):
            if fname.startswith('fontlist-') and fname.endswith('.json'):
                cache_file_path = os.path.join(cache_dir, fname)
                try:
                    os.remove(cache_file_path)
                    # st.info(f"Matplotlib 폰트 캐시 파일 삭제: {os.path.basename(cache_file_path)}") # 개발 중에는 유용하지만 배포 시에는 제거 가능
                except OSError as e:
                    st.warning(f"폰트 캐시 파일 삭제 실패 ({os.path.basename(cache_file_path)}): {e}")
    else:
        st.warning(f"Matplotlib 폰트 캐시 디렉토리를 찾을 수 없습니다: {cache_dir}")

    # 폰트 매니저가 새로운 캐시를 빌드하도록 force_read_cache=True로 초기화
    # 최신 Matplotlib 버전에서는 fm.fontManager.findSystemFonts() 호출만으로도 재빌드됨
    # fm._create_font_manager(force_read_cache=True) # 내부 함수 호출 대신 일반적인 방식으로
    ...
    if os.path.exists(FONT_PATH):
        ...
        fm.findSystemFonts(fontpaths=None, rebuild_cache=True) # 캐시를 확실히 재빌드
        return True
    else:
...
def create_network_analysis(text, keywords_dict, min_cooccurrence=1):
    ...
    for sentence in sentences:
        found_keywords = []
        # 대소문자 구분을 없애기 위해 문장과 키워드를 모두 소문자로 변환
